...
- **Naming**: `<NNN>` is the next free zero-padded 3-digit number (`ls docs/plans docs/plans/archive | grep -oE '^[0-9]{3}' | sort -n | tail -1` — archived numbers count, so they are never reused); `<task-title>` is a short kebab-case description of the task. Lexical order therefore matches chronological order.
- **Plans are append-only history.** Never renumber or delete an existing plan. If a plan changes materially mid-flight, update that same numbered file rather than adding a second one. Numbers are permanent.
- **Retention — `docs/plans/archive/`.** Superseded or obsolete plans are moved here. **Agents must ignore `docs/plans/archive/` by default**: do not read, search, index, or cite archived plans when working on a task. Treat them as historical record only, and open one solely when the user points at it explicitly.
- **Archiving is user-initiated only.** Never move a plan into `archive/` on your own initiative — not when a plan looks stale, superseded, or complete. Wait for an explicit user request. An archived plan keeps its original `<NNN>-<task-title>.md` name, and its number is never reused by a new plan.
