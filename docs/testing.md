...
- **jsdom resolves no SVG transforms.** GSAP cannot read an SVG element's `transform` attribute there and no-ops, so any behaviour that depends on a browser folding `translate(x y)` into a tween must be asserted in e2e, not in jsdom. Where a unit test needs that geometry it must seed it explicitly (see the cloud fixture in `timeline.test.ts`) and say so in a comment — a fixture that omits the authored transform silently models something the browser never produces.
- Ambient environment motion is off in e2e by default because `contextOptions.reducedMotion` is `'reduce'` suite-wide. A spec that needs it opts back in with `test.use({ contextOptions: { reducedMotion: 'no-preference' } })`; the `cloud drift` block in `e2e/environment.spec.ts` is the reference.
